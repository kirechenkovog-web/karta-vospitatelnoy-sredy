...
Стиль: уважительный, живой, поддерживающий, краткий, конкретный. Обращайся на «вы». Используй умеренное количество эмодзи там, где они уместны — особенно при перечислениях и переходах между шагами.
...
function getStagePrompt(stage: number, aspectTitle?: string, isAuto?: boolean, hints?: string[]): string {
  if (stage === 1 && aspectTitle) {
    const hintsBlock = hints && hints.length > 0
      ...
      : "";
    if (isAuto) {
      return `Пользователь только что открыл аспект «${aspectTitle}». Начни с Шага 1: поприветствуй, коротко скажи о чём аспект, затем озвучь 2–3 вспомогательных вопроса для размышления — поясни, что они необязательные, можно ответить или просто подумать. Затем предложи выставить оценку.${hintsBlock}`;
    }
